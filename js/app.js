...
const infoCardsOverviewLoad       = () => { return document.querySelector(".info-cards.section")}
const trendsContainerOverviewLoad = () => { return document.querySelector(".trends-container.section")}
const lgCardsOverviewLoad         = () => { return document.querySelector(".lg-cards.section")}
const announAndTrending           = () => { return document.querySelector(".announ-and-trending")}
const trends                      = () => { return document.querySelector(".trends")}
const dataContainer1              = () => { return document.querySelector(".data-container1")}
const dataContainer2              = () => { return document.querySelector(".data-container")}
const containerCards              = () => { return document.querySelector(".containerCards"); }
  ...
  searchComponent.addEventListener("click", () => {
    searchContainer().forEach(element => { 
      if(element.offsetParent === null) {
        ...
        recuperarUsers().length > 0 ? findUserSearchBar() :  alertaErrorUsuarios("warning", `No se encontró usuarios`) ; 
      } else  {
...
buttonDashboard().addEventListener("click", () => { 
  ...
  recuperarUsers().length > 0 ? displayLenguajesDelAmor(recuperarUsers()) : alertaErrorUsuarios("warning", `No se encontró usuarios`) 
});
...
favoritosUsers.addEventListener("click", () => {
  ...
  dataContainer2().style.display = "none";
  buttonOrdenarCards().style.display = "none"; 
  usersFavorite.length > 0 || recuperarUsersFavoritos().length > 0  ? userFavoriteLoad() : alertaErrorUsuarios("error", "No hay usuarios en favoritos") 
});
...
findUser.addEventListener("click", () => { 
  ...
  dataContainer1().style.display = "none";
  dataContainer2().style.display = "none";
  recuperarUsers().length > 0 ? findUserArray() : alertaErrorUsuarios("error", "No se encontro el usuario") 
});
...
addUser.addEventListener("click", () => {activarPopUpForm();});
 ...
 buttonOrdenarCards().addEventListener("click", () => { recuperarUsers() ? ordenar(recuperarUsers()) && alertaOrdenarDashboard() :  alertaErrorUsuarios("warning", `No se encontró usuarios`) });
...
allUsers.addEventListener("click", async () => {
  ...
  dataContainer2().style.display = "none";
  //botonOrdenarLoad();
  recuperarUsers().length > 0 || await usersLoadJSON().length > 0 ? usersLoad() && containerDashboardLoad() : alertaErrorUsuarios("warning", `No se encontró usuarios`) 
}); 
...
/*** Cards  */
const loadCardsInfo = () => {
  recuperarUsers().length > 0 ? totalFamily().textContent = recuperarUsers().length : totalFamily().textContent = 0;
  recuperarUsersFavoritos().length > 0 ? targetLove().textContent = recuperarUsersFavoritos().length : targetLove().textContent = 0;
  // Falta objetivos  
  // Falta recordatorios
}
...
const recuperarLenguajes = () => {
  if (localStorage.getItem("loveLanguages")) {
      let lenguajesRecuperados = JSON.parse(localStorage.getItem("loveLanguages"));
